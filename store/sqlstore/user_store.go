...
func (us SqlUserStore) GetForLogin(loginId string, allowSignInWithUsername, allowSignInWithEmail bool) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if allowSignInWithUsername && allowSignInWithEmail {
			builder = builder.Where("Username = :LoginId OR Email = :LoginId")
		} else if allowSignInWithUsername {
			builder = builder.Where("Username = :LoginId")
		} else if allowSignInWithEmail {
			builder = builder.Where("Email = :LoginId")
		} else {
		...
		builder = builder.Bind("LoginId", loginId)
...
func (us SqlUserStore) Search(teamId string, term string, options *model.UserSearchOptions) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		builder := usersQuery.
			...
			LimitInt(options.Limit)

		if teamId != "" {
			builder = builder.Join("TeamMembers tm ON ( tm.UserId = u.Id AND tm.DeleteAt = 0 AND tm.TeamId = :TeamId )").Bind("TeamId", teamId)
		}

		*result = us.performSearch(builder, term, options)
	})
}
...
func generateSearchQuery(builder *querybuilder.Builder, terms []string, fields []string, isPostgreSQL bool) *querybuilder.Builder {
...
func (us SqlUserStore) performSearch(builder *querybuilder.Builder, term string, options *model.UserSearchOptions) store.StoreResult {
	...
	if !options.AllowInactive {
		builder = builder.Where("u.DeleteAt = 0")
	}

	if strings.TrimSpace(term) != "" {
		isPostgreSQL := us.DriverName() == model.DATABASE_DRIVER_POSTGRES
		builder = generateSearchQuery(builder, strings.Fields(term), searchType, isPostgreSQL)
	}
